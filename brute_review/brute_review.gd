...
const CLIPS: Array[String] = ["idle", "move", "windup", "attack", "recovery", "hit", "death"]
...
func _ready() -> void:
	...
	var title := Label.new()
	title.text = "BRUTE / 112 FRAMES / ANIMATION REVIEW"
...
func play_clip(name: String) -> void:
	...
	clock = 0.0
	paused = false
	slider.max_value = Brute.Frames.get_frame_count(StringName(clip + "_e")) - 1
	slider.set_value_no_signal(0.0)
	queue_redraw()

func _scrub(frame: float) -> void:
	paused = true
	clock = frame / Brute.Frames.get_animation_speed(StringName(clip + "_e"))
	queue_redraw()
...
func _process(dt: float) -> void:
	...
	var fps := Brute.Frames.get_animation_speed(key)
	var count := Brute.Frames.get_frame_count(key)
	if not paused:
		clock += dt * speed
		clock = fposmod(clock, count / fps) if Brute.Frames.get_animation_loop(key) else minf(clock, count / fps)
	var frame := mini(count - 1, maxi(0, floori(clock * fps + 0.00001)))
	slider.set_value_no_signal(frame)
	status.text = "%s | FRAME %d/%d | %.1fx | %s | native / 1.2x | review candidate, not final approval" % [clip.to_upper(), frame+1, count, speed, "PAUSED" if paused else "PLAYING"]
	queue_redraw()
...
func _draw() -> void:
	...
	for i in range(4):
		...
		var image: Texture2D = Brute.texture(clip,directions[i],clock)
		draw_texture_rect(image,Rect2(point-Brute.PIVOT,Brute.CELL),false)
		point.y = 415
		draw_texture_rect(image,Rect2(point-Brute.PIVOT*1.2,Brute.CELL*1.2),false)
